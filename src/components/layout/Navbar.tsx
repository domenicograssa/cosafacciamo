    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Logo */}
          <Link href={href("/")} className="flex flex-col justify-center shrink-0">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600">
            ...
            <Link href={href("/organizzatori")} className="hover:text-gray-900 transition-colors">{t.nav.organizers}</Link>
            <Link href={href("/contatti")}  className="hover:text-gray-900 transition-colors">Contatti</Link>
          </nav>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href={href("/eventi")} className="p-2 text-gray-500 hover:text-gray-900 transition-colors" aria-label="Cerca eventi">
            ...
            <Link href={href("/accedi")} className="hidden sm:block text-gray-600 hover:text-gray-900 text-sm font-medium transition-colors">
              {t.nav.login}
            </Link>
            <Link href={href("/pubblica")} className="hidden sm:block bg-amber-400 hover:bg-amber-500 text-white text-sm font-semibold px-4 py-2 rounded-full transition-colors">
              {t.nav.publish}
            </Link>
        ...
        {menuOpen && (
          <nav className="md:hidden border-t border-gray-100 py-4 flex flex-col gap-3 text-sm font-medium text-gray-600">
            ...
            <Link href={href("/organizzatori")} className="px-2 py-1 hover:text-gray-900" onClick={() => setMenuOpen(false)}>{t.nav.organizers}</Link>
            <Link href={href("/contatti")}      className="px-2 py-1 hover:text-gray-900" onClick={() => setMenuOpen(false)}>Contatti</Link>
            <Link href={href("/accedi")}        className="px-2 py-1 hover:text-gray-900" onClick={() => setMenuOpen(false)}>{t.nav.login} / Dashboard</Link>
            <Link href={href("/pubblica")} className="mt-2 text-center bg-amber-400 text-white font-semibold px-4 py-2 rounded-full" onClick={() => setMenuOpen(false)}>
              {t.nav.publish}
            </Link>
            <button
