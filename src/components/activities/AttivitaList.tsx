...
export default function AttivitaList({ attivita, categorie, comuni, titoloIniziale }: AttivitaListProps) {
  ...
  const PannelloFiltri = () => (
    <div className="space-y-6">
      {/* Comune */}
      <div>
        <p className="text-sm font-semibold text-gray-900 mb-3">{t.activity.location}</p>
        <select
          value={filtri.comune}
        ...
        >
          <option value="">{t.activity.allLocations}</option>
          {comuni.map(c => (
      ...
      <div>
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm font-medium text-gray-700">{t.activity.freeOnly}</span>
        </label>
      </div>

      {/* Categorie */}
      <div>
        <p className="text-sm font-semibold text-gray-900 mb-3">{t.activity.category}</p>
        ...
        <div className="flex-1 min-w-0">
          {attivitaFiltrate.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="font-semibold text-gray-900">{t.activity.noActivitiesFound}</p>
              <p className="text-sm text-gray-500 mt-1">{t.activity.tryDifferentFilters}</p>
      ...
      {filtroAperto && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl p-5 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <p className="font-bold text-gray-900 text-lg">{t.activity.filters}</p>
            ...
            >
              {t.activity.show} {attivitaFiltrate.length} {t.activity.activitiesWord}
            </button>
          </div>
        </div>
      )}
